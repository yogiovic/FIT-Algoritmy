...
*/
	...
	if( s == NULL ) { //overime
		stackError(SERR_INIT); //ak zlyhalo, error a return
		return;
	}

	else s->top = -1; //inak nastavime index vrcholu
...
*/
	return s->top == -1; //ak je zasobnik prazdny
}
...
*/
	return s->top == STACK_SIZE - 1; //indexujeme od 0...
}
...
** funkci stackEmpty.
*/
	if( stackEmpty(s) == 1 ) { //pri prazdnom zasobniku nekorektne
		stackError(SERR_TOP);
		return;
	}
	
	else *c = s->arr[s->top]; //vratime prvok na vrchu
}
...
*/
	  ...
	  if( stackEmpty(s) == 0 ) s->top = s->top - 1; //...
}
...
*/
	if( stackFull(s) != 0 ) {
		...
		s->top = s->top + 1; //inkrementujeme
		s->arr[s->top] = c; //pridame
	}
